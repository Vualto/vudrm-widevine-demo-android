...
public class PlayerActivity extends AppCompatActivity
    ...
    protected boolean initializePlayer() {
        if (player == null) {
            ...
            // Check if asset requires a StudioDrmSessionManager
            if (mediaItems.get(0).playbackProperties.drmConfiguration != null) {
                if (StudiodrmHelper.useSdk(this, mediaItems.get(0).playbackProperties.drmConfiguration.licenseUri)) {
                    try {
                        Toast.makeText(getApplicationContext(), "setting up Studio DRM using SDK", Toast.LENGTH_LONG).show();

                        String token = StudiodrmHelper.getToken(mediaItems.get(0));
                        DrmSessionManager drmSessionManager = StudiodrmHelper.getStudioDrmSessionManager(
                                mediaItems.get(0).playbackProperties.uri.toString(), token);

                        mediaSourceFactory.setDrmSessionManager(drmSessionManager);
                    } catch (Exception e) {
                        Toast.makeText(getApplicationContext(), "error setting Studio DRM", Toast.LENGTH_LONG).show();
                        e.printStackTrace();
                    }
                }
            }
