...
public class DownloadTracker {
    ...
    private final class StartDownloadDialogHelper
        ...
        public void onPrepared(@NonNull DownloadHelper helper) {
            ...
            if ( StudiodrmHelper.useSdk(context, mediaItem.playbackProperties.drmConfiguration.licenseUri) ) {
                Toast.makeText(context, "setting up Studio DRM using SDK", Toast.LENGTH_LONG).show();

                try {
                    // example using Studio DRM Widevine SDK
                    DrmSessionManager drmSessionManager = StudiodrmHelper.getStudioDrmSessionManager(

                            mediaItem.playbackProperties.uri.toString(), /* should per asset */ StudiodrmHelper.getToken(mediaItem));
                    ...
                    Toast.makeText(context, "error setting Studio DRM callbacks" + e.getMessage(), Toast.LENGTH_LONG).show();
                }
            }
